...
export function Users(props: UsersProps) {
  ...
  return (
    <>
      ...
      <div>
        <h1 className="flex items-center text-center mb-6 text-2xl font-semibold text-gray-900 dark:text-white">Users</h1>
        <hr className="block mb-2 text-sm font-medium text-gray-900 dark:text-white"/>
        {data?.data?.users.map((user) => (
          <div key={user.id}>
            <p className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Name: {user.name}</p>
            <p className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Email: {user.email}</p>
            <p className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">ID: {user.id}</p>
            <hr className="block mb-2 text-sm font-medium text-gray-900 dark:text-white"/>
          </div>
        ))}
      </div>
